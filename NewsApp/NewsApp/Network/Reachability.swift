...
extension SCNetworkReachabilityFlags {
    ...
    var description: String {
        let wan = isOnWWANFlagSet ? "W" : "-"
        let reach = isReachableFlagSet ? "R" : "-"
        let con = isConnectionRequiredFlagSet ? "c" : "-"
        let tran = isTransientConnectionFlagSet ? "t" : "-"
        let inter = isInterventionRequiredFlagSet ? "i" : "-"
        let conTrf = isConnectionOnTrafficFlagSet ? "C" : "-"
        let conD = isConnectionOnDemandFlagSet ? "D" : "-"
        let loc = isLocalAddressFlagSet ? "l" : "-"
        let dir = isDirectFlagSet ? "d" : "-"

        return "\(wan)\(reach) \(con)\(tran)\(inter)\(conTrf)\(conD)\(loc)\(dir)"
    }
}
